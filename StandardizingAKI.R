...
addBackCalcAKI <- function(dataframe) {
  ...
  dataframe[, baseline_creat := .SD[time >= admission - as.difftime(365, units='days') & time <= admission - as.difftime(7, units='days') & inpatient == F, round(median(creat), 6)], by = mrn]
  dataframe[time < admission - as.difftime(6, units='hours'), baseline_creat := NA]
  
  #Add AKI stages
  dataframe[, stage1 := (creat >= round(1.5 * baseline_creat, digits=2))]
  dataframe[, stage2 := (creat >= 2 * baseline_creat)]
  dataframe[, stage3 := (creat >= 3 * baseline_creat)]
  dataframe[, bc := stage1 + stage2 + stage3]
  dataframe[is.na(bc), bc := 0]
}
